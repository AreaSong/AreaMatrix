...
struct ShareImportCoreFFIClient {
    ...
    private func ensureCurrentContract() throws {
        guard ffi_area_matrix_core_uniffi_contract_version() == 26,
            ...
            throw ShareImportError.unavailable("AreaMatrix Core binding contract mismatch.")
        }
    }
}

private enum ShareImportCoreFFIError: LocalizedError {
    case bufferOverflow
...
private func checkStatus(_ status: RustCallStatus) throws {
    ...
    case 2:
        if status.errorBuf.len > 0 {
            throw try ShareImportCoreFFIError.rustPanic(FFIReader.liftString(status.errorBuf))
        }
        try FFIReader.deallocate(status.errorBuf)
        throw ShareImportCoreFFIError.rustPanic("Rust panic")
    default:
        try FFIReader.deallocate(status.errorBuf)
        throw ShareImportCoreFFIError.unexpectedStatus(status.code)
    }
}
...
private enum FFIWriter {
    ...
    static func lowerImportOptions(_ request: ShareImportCoreRequest) throws -> RustBuffer {
        var bytes: [UInt8] = []
...
private enum FFIReader {
    static func liftCategoryPrediction(_ buffer: RustBuffer) throws -> ShareImportCategoryPrediction {
        var reader = Reader(buffer: buffer)
        let prediction = try ShareImportCategoryPrediction(
    ...
    static func liftCoreError(_ buffer: RustBuffer) throws -> ShareImportError {
        var reader = Reader(buffer: buffer)
        let variant = try reader.readInt32()
        let error: ShareImportError = switch variant {
        case 1, 12:
            try .io(reader.readString())
        case 6, 7:
            try .conflictNeedsReview(reader.readString())
        case 11:
    ...
    static func liftString(_ buffer: RustBuffer) throws -> String {
        ...
        guard let string = String(bytes: bytes, encoding: .utf8) else {
            throw ShareImportCoreFFIError.invalidStringEncoding
        }
        return string
    }
    ...
    private struct Reader {
        ...
        mutating func finish() throws {
            ...
            guard offset == data.count else {
                throw ShareImportCoreFFIError.incompleteData
            }
        }
        ...
        mutating func readString() throws -> String {
            ...
            guard count >= 0, data.count >= offset + count else {
                throw ShareImportCoreFFIError.bufferOverflow
            }
            defer { offset += count }
        ...
        mutating func readCoreErrorPayload(variant: Int32) throws -> String {
            ...
            default:
                throw ShareImportCoreFFIError.unexpectedEnumCase(Int64(variant))
            }
        }
        ...
        private mutating func readOptionalString() throws -> String? {
            ...
            case 1:
                return try readString()
            default:
                throw ShareImportCoreFFIError.unexpectedOptionalTag(tag)
            }
        }
        ...
        private mutating func readStorageMode() throws -> String {
            ...
            case 3:
                return "Indexed"
            case let value:
                throw ShareImportCoreFFIError.unexpectedEnumCase(Int64(value))
            }
        }
