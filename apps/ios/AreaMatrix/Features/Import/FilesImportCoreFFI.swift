...
private enum FilesImportCoreFFIError: LocalizedError {
    case bufferOverflow
...
private func checkStatus(_ status: RustCallStatus) throws {
    ...
    case 2:
        if status.errorBuf.len > 0 {
            throw try FilesImportCoreFFIError.rustPanic(FFIReader.liftString(status.errorBuf))
        }
        try FFIReader.deallocate(status.errorBuf)
        throw FilesImportCoreFFIError.rustPanic("Rust panic")
    default:
        try FFIReader.deallocate(status.errorBuf)
        throw FilesImportCoreFFIError.unexpectedStatus(status.code)
    }
}
...
private enum FFIWriter {
    ...
    static func lowerImportOptions(_ request: FilesImportCoreRequest) throws -> RustBuffer {
        var bytes: [UInt8] = []
...
private enum FFIReader {
    static func liftCategoryPrediction(_ buffer: RustBuffer) throws -> FilesImportCategoryPrediction {
        var reader = Reader(buffer: buffer)
        let prediction = try FilesImportCategoryPrediction(
    ...
    static func liftString(_ buffer: RustBuffer) throws -> String {
        ...
        guard let string = String(bytes: bytes, encoding: .utf8) else {
            throw FilesImportCoreFFIError.invalidStringEncoding
        }
        return string
    }
    ...
    private struct Reader {
        ...
        mutating func finish() throws {
            ...
            guard offset == data.count else {
                throw FilesImportCoreFFIError.incompleteData
            }
        }
        ...
        mutating func readString() throws -> String {
            ...
            guard count >= 0, data.count >= offset + count else {
                throw FilesImportCoreFFIError.bufferOverflow
            }
            defer { offset += count }
        ...
        mutating func readCoreErrorPayload(variant: Int32) throws -> String {
            ...
            default:
                throw FilesImportCoreFFIError.unexpectedEnumCase(Int64(variant))
            }
        }
        ...
        private mutating func readOptionalString() throws -> String? {
            ...
            case 1:
                return try readString()
            default:
                throw FilesImportCoreFFIError.unexpectedOptionalTag(tag)
            }
        }
        ...
        private mutating func readStorageMode() throws -> String {
            ...
            case 3:
                return "Indexed"
            case let value:
                throw FilesImportCoreFFIError.unexpectedEnumCase(Int64(value))
            }
        }
