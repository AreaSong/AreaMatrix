...
public sealed class MissingFileRecoveryViewModel : INotifyPropertyChanged
{
    ...
    public void SelectRelinkPath(string selectedPath)
    {
        SelectedRelinkPath = selectedPath;
        Error = null;
        Report = null;
    }

    public void RecordLocateFileFailure(Exception exception)
    {
    ...
    private async Task RunActionAsync(Func<Task<MissingFileRecoveryReport>> action)
    {
        IsWorking = true;
        Error = null;
        try
