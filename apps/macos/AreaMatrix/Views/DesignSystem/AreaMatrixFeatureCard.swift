...
private struct LucideGlobeShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        let ellipse = Path { p in
            p.addEllipse(in: CGRect(x: 6, y: 2, width: 12, height: 20))
        }
...
private struct LucideCheckCircleShape: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        path.addEllipse(in: CGRect(x: 2, y: 2, width: 20, height: 20))
        path.move(to: CGPoint(x: 9, y: 12))
...
private struct LucideXCircleShape: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        path.addEllipse(in: CGRect(x: 2, y: 2, width: 20, height: 20))
...
private struct LucideAlertTriangleShape: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
...
private struct LucideMoreHorizontalShape: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        path.move(to: CGPoint(x: 5, y: 12))
...
private struct LucideHardDriveShape: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
...
private struct LucideFolderCogShape: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
...
private struct LucideCloudShape: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        ...
        path.addArc(center: CGPoint(x: 12, y: 10), radius: 6, startAngle: .degrees(0), endAngle: .degrees(180), clockwise: true)
...
private struct LucideRefreshCcw: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        path.addArc(center: CGPoint(x: 12, y: 12), radius: 9, startAngle: .degrees(45), endAngle: .degrees(315), clockwise: true)
...
private struct LucideClock: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        path.addArc(center: CGPoint(x: 12, y: 12), radius: 10, startAngle: .zero, endAngle: .degrees(360), clockwise: false)
...
private struct LucideFilePlus2: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
...
private struct LucideFolderOpen: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
