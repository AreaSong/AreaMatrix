        ...
        case .rootChanged, .startupFailed:
            let mapping = CoreErrorMappingSnapshot(
                ...
                userMessage: request.kind == .rootChanged
                    ? L10n.message("Repository folder is unavailable.")
                    : L10n.message("Filesystem monitoring could not start."),
                severity: .high,
                suggestedAction: request.kind == .rootChanged
                    ? L10n.message("Reconnect the repository folder before continuing.")
                    : L10n.message("Retry opening the repository. If the problem persists, collect diagnostics."),
