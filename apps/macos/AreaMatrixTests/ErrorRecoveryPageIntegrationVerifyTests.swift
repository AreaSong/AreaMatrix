...
final class ErrorRecoveryPageIntegrationVerifyTests: XCTestCase {
    @MainActor
    func testStartupRecoveryPageIntegrationConnectsStartupRecoveryMappingEntryExitAndTreeState() async {
        ...
        guard let failedState = requireMainLoadingState(
            model,
            message: "Expected startup recovery failure to remain on startup-recovery main loading error recovery"
        ) else { return }

        await recoverer.assertRequestedRepoPaths(["/tmp/startupRecovery-repo"])
        ...
        await treeLister.assertRequestedRepoPaths(["/tmp/startupRecovery-repo"])
        XCTAssertEqual(
            model.route,
    ...
    func testStartupRecoveryPageIntegrationRoutesFatalDbMappingToRepairWithoutRunningRepair() async {
        ...
        await recoverer.assertRequestedRepoPaths(["/tmp/startupRecovery-corrupt"])
...
private func assertStartupRecoveryRetryingState(
    ...
    await recoverer.assertRequestedRepoPaths(["/tmp/startupRecovery-repo", "/tmp/startupRecovery-repo"])
    await opener.assertRequestedConfiguredRepoPaths(["/tmp/startupRecovery-repo"])
