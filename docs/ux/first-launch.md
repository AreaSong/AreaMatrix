...
- **首启可完成**：新用户首次启动从点击 App 到进入主界面（可拖拽）≤ 60 秒（不含下载 iCloud 文件）。
- **不可写路径可恢复**：选择不可写路径时给出明确原因与“更换路径”按钮，无 crash。
- **iCloud 风险需确认**：选择 iCloud 路径时弹“iCloud 风险提示”，用户必须明确确认（一次性）。
- **初始化中断可恢复**：初始化过程被强制终止（kill app）后，下次启动能自动恢复到“继续初始化/清理残留”。
- **键盘可完成**：向导所有页面支持键盘（Tab/Enter/Esc）完成主流程。
- **非空目录接管安全**：选择非空目录时进入“接管已有目录”流程，完成后已有 `README.md` 和项目文件保持不变。
...
- 如果取消实现成本过高：当前可先不提供 Cancel，但要确保初始化极快。
