...
- `选择本地文件夹`：快捷键 `Command-O`，打开只允许选择目录的 `NSOpenPanel`。
...
Welcome 的动效和扫描文案只用于过渡到文件夹选择器。选择目录前不会写入用户资料。
...
- 非空但未初始化目录：可以接管已有目录。
- 已初始化资料库：可以直接打开。
- 不可写、路径缺失、metadata 损坏或存在未完成扫描：显示对应恢复动作。
...
接管时 UI 可以读取 scan session 显示已扫描、已插入、已更新、冲突、不可读和跳过数量。无法读取进度不会被当作初始化成功。
