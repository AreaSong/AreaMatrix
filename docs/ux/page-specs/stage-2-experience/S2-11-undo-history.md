...
- 键盘：历史列表支持方向键浏览，`Undo latest`、`Redo latest`、`Review details` 和 Close 可 Tab 到达。
- 焦点：面板打开时焦点落在最新操作；关闭、Undo 或 Redo 完成后焦点回到打开前上下文。
- VoiceOver：读出每条历史的类型、时间、影响数量、可用性、blocked/expired 原因。
- 错误关联：Trash restore 失败、外部变更阻塞和过期状态必须关联到对应历史行。
- 状态表达：Blocked、Expired、Redo available 和图标类型不能只靠颜色或图标。
