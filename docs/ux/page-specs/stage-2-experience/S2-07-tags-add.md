...
- 键盘：输入框支持回车选择高亮候选，chip 删除按钮可 Tab 到达，`Esc` 关闭 popover。
- 焦点：打开后焦点落在输入框；关闭或操作成功后焦点回到 `+ Add...` 或触发的 chip。
- VoiceOver：读出当前标签、候选标签、已添加状态、Create 行和 Suggestions 入口。
- 错误关联：非法字符、Tag store 加载失败、创建失败和关系写入失败必须关联到输入框或候选行。
- 状态表达：chip 颜色、最近标签和已添加状态不能只靠颜色；需要文字或可读状态。
