...
5. 点击 Save rule only 写入规则配置或分类器编辑草稿并返回；不更新现有文件分类。
6. 点击 Save and apply 执行批量分类更新或删除分类 replacement 更新，显示进度和结果。
7. 执行中显示 `Applying rule...`，禁止重复提交。
8. 部分失败时显示成功、失败、阻塞数量；成功项进入 Undo stack。
