...
| F4 | 自动 README | 每个分类目录维护 README.md，根目录有总 README |
| F5 | 树状图导航 | 侧边栏完整目录树 |
| F6 | 文件列表 | 主区表格展示选中节点下的文件 |
| F7 | 详情面板 | 元数据 + 改动时间线 + 伴生笔记 |
| F8 | 改动日志 | SQLite 持久化所有操作 |
| F9 | FSEvents 同步 | 外部修改实时回流 UI 与 DB |
| F10 | 事务式导入 | 任何中断不留半文件 |
| F11 | 去重 | SHA256 检测重复，提示用户 |
| F12 | 设置面板 | 资料库路径、默认存储模式 |
| F13 | 首次启动向导 | 选位置、初始化资料库结构 |
| F14 | iCloud 兼容 | 检测 iCloud 路径，处理占位符 |
...
3. **不绑架**：用户可以随时弃用本工具，资料库目录在标准 Finder 里仍可访问
