...
enum RuleImpactMatchReason { "Keyword", "Extension" };
...
- Stage 2：C2-01 `search_files`、C2-02 `list_filter_facets` 和 C2-03 saved search
  ...
  （`save_classifier_rule`）已提升为本文与
  `core/area_matrix.udl` 的稳定合同；
  Redo、导入冲突批量决策、非 AI 标签建议、分类规则编辑、规则影响预览仍见
...
`Preview rule impact` dry-run。输入是已初始化 `repoPath` 和一个 `ClassifierRule`
草稿。`target_category`、`keywords`、`extensions`、`priority` 的校验语义与
`save_classifier_rule` 保持一致；`keywords` 和 `extensions` 是独立 matcher basis，
不是 keyword AND extension 复合规则。

输出 `RuleImpactReport`：

- `rule`：回显本次预览的规则草稿，供规则摘要和 Back 恢复。
...
- 只读读取 classifier 配置和文件 metadata；可以复用当前 `classifier.yaml`
  matcher 语义、DB 查询和 move dry-run 的冲突检测语义。
...
- `Config`：`repoPath`、目标分类、关键词、扩展名、priority 或 classifier schema 无效。
