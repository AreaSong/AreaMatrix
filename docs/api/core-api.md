...
- Stage 2：C2-01 `search_files`、C2-02 `list_filter_facets` 和 C2-03 saved search
  ...
  以及 C2-07 undo action log（`list_undo_actions`、`undo_action`）和 C2-08
  批量改分类（`preview_batch_move_to_category`、`batch_move_to_category`）已提升为
  本文与 `core/area_matrix.udl` 的稳定合同；
  Redo、批量删除/重命名、导入冲突批量决策、非 AI 标签建议、分类规则编辑仍见
