...
> v1 的错误恢复发布证据：从 Core 错误码追到 UX 文案、恢复动作、诊断入口、阻断级别和测试证据。
>
> 阅读时长：约 10 分钟。
...
| 等级 | 含义 | 发布处理 |
|---|---|---|
| P0 | 可能丢失、覆盖、移动用户文件，或失败后留下最终目录半成品 | 阻断 v1 发布，必须先修复 |
| P1 | 用户无法完成明确恢复动作，或 Core/UX 语义冲突会误导恢复路径 | 阻断 v1 发布，需要回到对应版本任务补齐 |
...
- 未发现没有 Core 来源的 v1 用户错误页面；DB locked/corrupted、磁盘满、EBUSY 属于 `Db`/`Io` 的子语义。
...
当前 P0 结论：未发现已知事务式导入 P0 缺口。若上述任一测试失败，v1 发布必须直接阻断。
