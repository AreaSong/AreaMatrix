...
> 本 playbook 只索引和解释现有规则，不是新的 source of truth。产品语义仍看 `docs/**`，治理边界仍看 `.ai-governance/**`，live execution 仍只从已批准的 `workflow/versions/<version>/execution/**` 和 `./task-loop` 进入。
...
大功能 / 新 v* -> workflow/**
live 执行 -> workflow/versions/<version>/execution/** + ./task-loop
```
...
| 新版本、大功能、重构、优化生命周期 | `workflow/**` | `./dev workflow init --version <v*>`、`./dev workflow doctor` | 从 workflow 绕过 promotion 直接写 live `workflow/versions/<version>/execution/**` 或抢占 live label |
| 已批准小任务的执行和验收 | `workflow/versions/<version>/execution/**` + `./task-loop` | `python3 workflow/versions/v1-mvp/execution/_shared/prompt_pipeline.py doctor`、`./dev`、`./task-loop` | 启动第二 runner、手改 runtime state、绕过 verify-ready |
...
| Automations / Cloud / Worktrees / SDK / app-server / remote-control | 暂不接管 AreaMatrix 主线 | 不创建第二套 runner、progress、queue、checkpoint 或 state；如要影响主线，先走 workflow planning gate 和人工确认 |
...
| Execution 污染 | 是否新增 runner、自动执行 backlog、直接 promotion、或让 hooks / automation / subagent 接管 `./task-loop`？ | live execution 只从批准的 `workflow/versions/<version>/execution/**` 和 `./task-loop` 进入 |
| State 污染 | 是否写 `workflow/versions/<version>/execution/_shared/progress.json`、logs、run summaries、lock、checkpoint、branch/commit 状态？ | backlog、workflow preview、references 和 skills 不写 runtime state |
