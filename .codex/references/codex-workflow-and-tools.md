...
| Repo-local Skills | `.codex/skills-src/**`、`.agents/skills/**` | 已有 | 已有 9 个 AreaMatrix skills，且 `./dev check skills` / `./dev check quality` 通过 | 继续强化内容 | 把重复治理知识固化为可复用视角 | skill 语义变更先同步 `.ai-governance/` | P1 持续 |
...
| P1 | AreaMatrix skills 强化 | 强化现有 9 个 repo-local skills 的触发和边界 | AreaMatrix 当前 skills | skills-src / index / validation matrix / quality smoke 的小步补强 | 不新增重复 skill；先复用现有 skill owner |
