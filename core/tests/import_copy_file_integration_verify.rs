...
use area_matrix_core::{
    import_file, init_repo, list_files, list_tree_json, CoreError, DuplicateStrategy, FileFilter,
...
#[test]
fn import_copy_file_integration_verify_real_copy_supports_consuming_context() {
    ...
    assert_eq!(
        fs::read(repo.path().join(&entry.path)).expect("read final imported file"),
        source_before
    );
...
#[test]
fn import_copy_file_integration_verify_failure_keeps_source_and_final_state_safe() {
    let repo = initialized_repo();
    let (_source_root, source) = source_file("invoice.pdf", b"invoice bytes");
