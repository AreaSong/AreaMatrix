...
fn indexed_auto_options() -> ImportOptions {
    ImportOptions {
...
fn indexed_selected_directory_options(directory: &str) -> ImportOptions {
    ImportOptions {
        mode: StorageMode::Indexed,
...
fn assert_file_row_matches_indexed_source(repo: &Path, file_id: i64, source_path: &str) {
...
fn assert_change_log_matches_indexed_import(
    repo: &Path,
    file_id: i64,
    source_path: &str,
    destination: &str,
    ...
    assert_eq!(detail["destination"], destination);
    assert_eq!(detail["by"], "user");
}

#[test]
fn import_index_file_validation_proves_success_without_copying_or_moving_source() {
    let repo = initialized_repo();
    let (_source_root, source) = source_file("invoice.pdf", b"invoice bytes");
    let source_path = path_string(&source);
    let source_bytes = fs::read(&source).expect("read source before indexed import");

    let entry = import_file(
        path_string(repo.path()),
        source_path.clone(),
        indexed_selected_directory_options("finance/2026"),
    )
    .expect("index source file");
    ...
    let listed = list_files(path_string(repo.path()), empty_filter()).expect("list indexed files");
    assert_eq!(listed, vec![entry.clone()]);
    assert_file_row_matches_indexed_source(repo.path(), entry.id, &source_path);
    assert_change_log_matches_indexed_import(
        repo.path(),
        entry.id,
        &source_path,
        "selected_directory",
...
#[test]
fn import_index_file_validation_missing_source_is_file_not_found_without_side_effects() {
    let repo = initialized_repo();
...
#[test]
fn import_index_file_validation_rejects_metadata_internal_source_without_side_effects() {
    let repo = initialized_repo();
...
#[test]
fn import_index_file_validation_rejects_metadata_destination_without_copying() {
    let repo = initialized_repo();
...
#[test]
fn import_index_file_validation_duplicate_skip_keeps_source_and_existing_state() {
    let repo = initialized_repo();
    ...
    assert_eq!(
        fs::read(&source_b).expect("read duplicate source after rejected indexed import"),
        b"same bytes"
    );
    assert!(!repo.path().join("finance").exists());
...
#[test]
fn import_index_file_validation_list_files_keeps_deleted_external_source_metadata() {
    let repo = initialized_repo();
