...
use area_matrix_core::{
    init_repo, CoreError, OverviewOutput, RepoInitMode, RepoInitOptions, SearchFacetCount,
    SearchFacetQuery, SearchFacets, SearchFilter, SearchPagination, SearchResultPage, SearchScope,
...
const CAPABILITY_SPEC: &str =
    include_str!("../../../workflow/versions/v2/source-docs/core/capability-specs/stage-2-experience/C2-02-search-filters.md");
...
const SEARCH_FILTERS_PAGE: &str =
    include_str!("../../../workflow/versions/v2/source-docs/ux/page-specs/stage-2-experience/S2-02-search-filters.md");
...
pub(crate) fn default_query() -> SearchFacetQuery {
    SearchFacetQuery {
        query: String::new(),
...
pub(crate) fn seed_combined_facets(repo: &Path) {
...
pub(crate) fn combined_query() -> SearchFacetQuery {
...
pub(crate) fn search_filter_from_query(query: &SearchFacetQuery) -> SearchFilter {
    SearchFilter {
        scope: query.scope.clone(),
        current_path: query.current_path.clone(),
        category: query.category.clone(),
        file_kind: query.file_kind.clone(),
        tags: query.tags.clone(),
        tag_match_mode: query.tag_match_mode.clone(),
        imported_after: query.imported_after,
        imported_before: query.imported_before,
        modified_after: query.modified_after,
        modified_before: query.modified_before,
        storage_mode: query.storage_mode.clone(),
...
pub(crate) fn assert_combined_facets(facets: &SearchFacets) {
...
pub(crate) fn assert_config_error(result: Result<SearchFacets, CoreError>) {
    assert!(matches!(result, Err(CoreError::Config { .. })));
}

pub(crate) fn assert_search_config_error(result: Result<SearchResultPage, CoreError>) {
    assert!(matches!(result, Err(CoreError::Config { .. })));
}
...
pub(crate) fn assert_rust_contract_alignment() {
    for fragment in [
        "pub fn search_files(",
