...
fn edited_note_detail(repo: &Path, file_id: i64) -> Value {
...
#[test]
fn read_write_note_implementation_writes_db_sidecar_and_change_log_consistently() {
    ...
    let detail = edited_note_detail(repo.path(), file_id);
...
#[test]
fn read_write_note_implementation_restores_old_sidecar_when_db_log_fails() {
    let repo = initialized_repo();
    let file_id = insert_active_file(repo.path(), "finance/report.pdf");
    let old_content = "old note";
    let new_content = "new note";
...
#[test]
fn read_write_note_implementation_allows_read_but_rejects_write_when_target_file_is_missing() {
    let repo = initialized_repo();
    let relative_path = "finance/report.pdf";
    let file_id = insert_active_file(repo.path(), relative_path);
    let content = "saved note";
    insert_note_row(repo.path(), file_id, content);
    fs::write(sidecar_path(repo.path(), relative_path), content).expect("write sidecar note");
    fs::remove_file(repo.path().join(relative_path)).expect("remove target file");
...
#[test]
fn read_write_note_implementation_does_not_overwrite_untracked_sidecar_file() {
    let repo = initialized_repo();
    let relative_path = "finance/report.pdf";
    let file_id = insert_active_file(repo.path(), relative_path);
...
#[test]
fn read_write_note_implementation_rejects_inconsistent_db_and_sidecar_state() {
    let repo = initialized_repo();
    let relative_path = "finance/report.pdf";
    let file_id = insert_active_file(repo.path(), relative_path);
