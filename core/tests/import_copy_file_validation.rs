...
fn selected_directory_options(directory: &str) -> ImportOptions {
    ImportOptions {
        mode: StorageMode::Copied,
...
fn assert_change_log_matches_import(repo: &Path, file_id: i64, source: &Path) {
    let connection = open_db(repo);
    let (action, detail_json): (String, String) = connection
        ...
        .expect("read import change log row");
...
#[test]
fn import_copy_file_validation_proves_success_fs_db_and_change_log_consistency() {
    let repo = initialized_repo();
    let (_source_root, source) = source_file("invoice.pdf", b"invoice bytes");
    let original_source_bytes = fs::read(&source).expect("read source before import");

    let entry = import_file(
        path_string(repo.path()),
        path_string(&source),
        selected_directory_options("finance/2026"),
    )
    .expect("import copied file into selected directory");
    ...
    assert_eq!(
        fs::read(repo.path().join(&entry.path)).expect("read active imported file"),
        original_source_bytes
    );
...
#[test]
fn import_copy_file_validation_rejects_metadata_internal_source_without_side_effects() {
    let repo = initialized_repo();
...
#[test]
fn import_copy_file_validation_rejects_metadata_destination_without_copying() {
    let repo = initialized_repo();
...
#[test]
fn import_copy_file_validation_duplicate_skip_keeps_existing_state() {
    let repo = initialized_repo();
