...
| 版本号 | 部分通过 | `core/Cargo.toml` 与 Xcode `MARKETING_VERSION` 当前为 `0.1.0`；Xcode 项目默认 `CURRENT_PROJECT_VERSION` 仍为 `202605101812`；`v0.1.0-unnotarized-preview.2` 产物通过 `./dev release local-qa --build-number 202606161707` 覆盖构建号；正式 `v0.1.0` tag 尚未创建 | 可以为未公证预览版创建 `v0.1.0-unnotarized-preview.2` tag；不得把它当作正式 `v0.1.0` release tag |
...
| 反馈渠道 | 部分通过 | Stage 1 MVP 指向 GitHub Discussions；`.github/ISSUE_TEMPLATE/alpha_feedback.md` 已提供可信测试者反馈入口，现有 `.github/ISSUE_TEMPLATE/config.yml` 已链接 Q&A / Ideas Discussions 和 Security Advisory；`alpha-feedback-route.md` 已把 release decision record、关闭条件和 pending 字段结构化。Alpha feedback issue template 已存在；可信测试者名单、正式公告或 Discussion 链接、最终反馈分流和 owner 决策仍未记录 | tester 入口已具备基础 issue template，但正式 alpha 仍缺 tester 名单和发布决策下的反馈路线闭环 |
...
| P1-RL-003 | P1 | Developer ID 签名/公证预检已补可复现命令，但当前项目无付费 Apple Developer Program，环境 BLOCKED；正式 DMG、公证、干净 Mac 首启证据仍缺失；2026-06-16 已生成 `v0.1.0-unnotarized-preview.2` 未公证预览 DMG，可作为 GitHub prerelease 给可信测试者 | 2026-07-06 `./dev release preflight` 退出码 1：无 valid Developer ID Application signing identity，`AC_PASSWORD` notarytool keychain profile 不可用；ad-hoc signed `.app`、同机 AppleScript/scroll smoke、`workflow/versions/v1-mvp/evidence/artifacts/AreaMatrix-0.1.0-local-qa.dmg` 或 `workflow/versions/v1-mvp/evidence/artifacts/AreaMatrix-v0.1.0-unnotarized-preview.2.dmg` 不能替代 Developer ID codesign、notarytool accepted log、stapler、正式 DMG checksum、DMG 公证日志和干净 Mac 首启记录 |
| P1-RL-004 | P1 | CHANGELOG / release notes / build number 已完成；preview tag `v0.1.0-unnotarized-preview.2` 可创建为 GitHub prerelease；正式 `v0.1.0` tag 尚未创建，状态为 `pending，不创建` | release candidate 提交、正式 alpha 门禁关闭后再创建并推送 `v0.1.0` tag；preview GitHub prerelease 不等于正式 GitHub Release |
...
| P1-RL-006 | P1 | Alpha feedback issue template 已补齐，`alpha-feedback-route.md` 已结构化 release decision record；tester 名单、正式 announcement Discussion 和最终反馈分流决策仍 pending | `.github/ISSUE_TEMPLATE/alpha_feedback.md` 提供可信测试者反馈字段和数据安全确认；`alpha-feedback-route.md` 要求记录 trusted tester list、release announcement / Discussion 链接、feedback route 和 triage owner；正式 alpha 前仍需 release owner 填入真实决策 |
