...
| Swift 物理模块化 | 已证明 | `AreaMatrixCoreContracts`、`AreaMatrixCoreBridgeContract`、`AreaMatrixCoreBridgeRuntime`、`AreaMatrixUIFoundation`、`AreaMatrixPlatformKit` 已作为 Swift Package 被 App target 真实链接；UIFoundation 已承载主题、路径、状态、步骤头、页面玻璃面板 / 工作区壳层、通用动作控件、Feature Card、品牌场景原语、主题资源表面、Lucide 图标路径、场景转场 / 视差环境、场景进入动效和解码文字，并由 Onboarding、Import、MainList、Settings、RepositoryLifecycle、主窗口路由和多组场景 Diorama 真实调用；PlatformKit 已承载 HTTPS 外链与 LocalFileURL 平台合同及默认实现，并由 Package/Xcode 双重测试覆盖；`CoreBridgeBoundary`、runtime coordinator 和 `CoreBridgeGeneratedAdapter` 的 boundary guard 已有 Package/Xcode 双重验证；平台能力、绑定合同 snapshot、稳定 Bridge capability protocol、diagnostics snapshot / collector contract、change-log listing contract、classification contract、search facet contract、AI call-log contract、tag value / tag suggestion contract、repository lifecycle contract 与 undo/redo capability contract 已迁入 `AreaMatrixCoreContracts` / `AreaMatrixCoreBridgeContract`，Bridge 仅保留生成 DTO 转换、本地化显示扩展和 App 兼容 typealias；AI call-log 的 `list_ai_calls` / `clear_ai_call_log`、search facet 的 `list_filter_facets`、tag value 与 tag suggestion 转换也已通过声明边界和生成适配器；所有已迁移的生成调用都经实例化适配器和声明边界校验；其余 feature-specific Bridge contract 已登记在 `docs/architecture/macos-frontend-architecture.md` 的 App-owned Bridge 保留清单，并由 `MacOSCoreBridgePackageGovernanceTests` 验证 owner、保留理由和退出条件；不再以微型 Package 数量作为完成标准 | 保持 Package contract 与 App-owned Bridge 清单同步；新增跨 Feature 稳定能力必须满足第二个真实调用方和独立验证后再迁移，禁止回流 UniFFI 或无理由增加微型 Package |
