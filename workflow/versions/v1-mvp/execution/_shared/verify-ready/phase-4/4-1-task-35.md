# Verify-ready Prompt: 4-1/task-35
...
- 类型：单任务验收
- 任务类型：`integration`
- 任务细分：`core-integration-verify`
- Phase：`phase-4`
- Task 标识：`4-1/task-35`
- Task 文件：`./workflow/versions/v1-mvp/execution/phase-4/4-1-stage2-experience/task-35-c2-07-integration-verify.md`
...
- 工程质量规则：`./workflow/versions/v1-mvp/execution/_shared/engineering-quality-rules.md`
- 编码规范：`./docs/development/coding-standards.md`
- Repo-local Skills：`./.codex/skills-src`
- Validation Driver：`./.codex/skills-src/areamatrix-validation-driver/SKILL.md`
- 依赖关系：`./workflow/versions/v1-mvp/execution/_shared/dependency-graph.md`
- Phase Manifest：`./workflow/versions/v1-mvp/execution/_shared/manifests/phase-4.md`
- Manifest 章节：`## 4-1/task-35`
- 依赖任务：`4-1/task-34`
- 风险等级：`Mission-Critical`
- 绑定 UX 页面：`S2-09-batch-add-tags, S2-10-undo-toast, S2-11-undo-history, S2-12-batch-change-category, S2-13-batch-delete-confirm, S2-14-batch-rename, S2-21-import-conflict-batch, S2-22-redo`
- 绑定 Core 能力：`C2-07`
- Control map 期望 Core 能力：`S2-09: C2-06, C2-07; S2-10: C2-07; S2-11: C2-07; S2-12: C2-08, C2-07; S2-13: C2-09, C2-07; S2-14: C2-10, C2-07; S2-21: C2-17, C2-07; S2-22: C2-18, C2-07`
- 当前 task 覆盖 Core 能力：`C2-07`
- Secondary capability 状态：`消费页面中的其他能力不属于当前 Core task 验收范围；仅检查当前能力是否满足这些页面对该能力的需求`
- Control map 之外能力：`None`
- 是否允许修改文件：`否，本模式只读验收`
- Manifest 计数：文档 `11` 个，现有代码 `2` 项，预期新增路径 `3` 项，禁止触碰路径 `1` 项，验证命令 `1` 个
...
1. 读取 task 文件：`./workflow/versions/v1-mvp/execution/phase-4/4-1-stage2-experience/task-35-c2-07-integration-verify.md`
2. 读取共享规则：`./workflow/versions/v1-mvp/execution/_shared/audit-rules.md`
3. 读取任务切片规则：`./workflow/versions/v1-mvp/execution/_shared/task-slicing-rules.md`
4. 读取工程质量规则：`./workflow/versions/v1-mvp/execution/_shared/engineering-quality-rules.md`
5. 读取编码规范：`./docs/development/coding-standards.md`
6. 读取 validation-driver：`./.codex/skills-src/areamatrix-validation-driver/SKILL.md`；不要读取 `~/.codex/skills-src/...`。
7. 读取依赖关系：`./workflow/versions/v1-mvp/execution/_shared/dependency-graph.md`
8. 读取 phase manifest：`./workflow/versions/v1-mvp/execution/_shared/manifests/phase-4.md`
9. 在 manifest 中定位章节：`## 4-1/task-35`
...
- Skill 源事实目录：`./.codex/skills-src`
- Skill 发现入口：`./.agents/skills`
- 禁止猜测全局路径：不要读取 `~/.codex/skills-src/...`。
- 若需要使用 AreaMatrix repo-local skill，只读取以下路径：
  - `areamatrix-task-loop`：`./.codex/skills-src/areamatrix-task-loop/SKILL.md`
  - `areamatrix-validation-driver`：`./.codex/skills-src/areamatrix-validation-driver/SKILL.md`
  - `areamatrix-doc-sync`：`./.codex/skills-src/areamatrix-doc-sync/SKILL.md`
  - `areamatrix-file-safety`：`./.codex/skills-src/areamatrix-file-safety/SKILL.md`
...
7. [workflow/versions/v1-mvp/execution/README.md](../../../README.md)
8. [workflow/versions/v1-mvp/execution/_shared/engineering-quality-rules.md](../../engineering-quality-rules.md)
9. The nearest path-local `AGENTS.md` for changed files, when present.
10. [workflow/residuals/README.md](../../../../../../residuals/README.md) when validation or completion claims depend on unresolved, deferred, or accepted residual items.

## References

- [references/validation-matrix.md](../../../../../../../.codex/skills-src/areamatrix-validation-driver/references/validation-matrix.md): path-to-command validation mapping.
- [references/report-format.md](../../../../../../../.codex/skills-src/areamatrix-validation-driver/references/report-format.md): required validation report structure.
- [../../references/completion-evidence-checklist.md](../../../../../../../.codex/references/completion-evidence-checklist.md): evidence checklist before claiming done, fixed, passing, merge-ready, or deliverable.
- [../../references/ui-evidence-tool-templates.md](../../../../../../../.codex/references/ui-evidence-tool-templates.md): Browser / Chrome / Computer Use trigger conditions, forbidden actions, and UI evidence format.
- [../../references/debugging-failure-attribution-runbook.md](../../../../../../../.codex/references/debugging-failure-attribution-runbook.md): decide whether a failure belongs to validation or another task-loop / drift / safety layer.
- [../areamatrix-doc-sync/SKILL.md](../../../../../../../.codex/skills-src/areamatrix-doc-sync/SKILL.md): source-of-truth and drift checks when docs, APIs, UDL, prompts, or Codex materials change.
- [../areamatrix-file-safety/SKILL.md](../../../../../../../.codex/skills-src/areamatrix-file-safety/SKILL.md): additional evidence for user-file, DB, staging, recovery, reindex, FSEvents, or iCloud risks.
- [../areamatrix-enterprise-governance/SKILL.md](../../../../../../../.codex/skills-src/areamatrix-enterprise-governance/SKILL.md): review, security, dependency, CI, and ownership gates.
- [../areamatrix-residual-ledger/SKILL.md](../../../../../../../.codex/skills-src/areamatrix-residual-ledger/SKILL.md): report whether relevant residual items remain open, blocked, deferred, or accepted.
...
- 当 verify-ready 由 `./task-loop` 调用时，本验收发生在 runner 写入 `completed` progress、summary 和 Git checkpoint 之前。
- 完整任务流程是：copy-ready 实现 -> task-scoped check 验证实现证据 -> verify-ready 只读验收 -> `VERIFY_RESULT: PASS` 后 runner 才写 completed progress、summary 和 Git checkpoint。
- 不得仅因为 `progress.json` 仍是 `in_progress`、新增文件尚未 `git add`、或 `git_checkpoint_status` 尚未写入而判定不通过。
- 这些 runner 收口证据会在本验收输出 `VERIFY_RESULT: PASS` 后由 task-loop 继续写入；若收口失败，应归因到 runner checkpoint 阶段。
- 仍需严格阻断与本 task 无关、危险或无法解释的脏改动，以及 Forbidden Touches、验证失败、代码质量、安全、隐私、依赖、CI 或 review blocker。

## Task-scoped 验证分层

- 普通 atomic Core task 的完成门禁是 `./dev check task <label>`，默认只跑 prompt doctor、diff check 和该能力映射的精确 Rust test target。
- Core capability integration verify 会在精确 Rust test target 之外加跑 Core quality gate：`cargo fmt --all -- --check` 和 `cargo clippy --all-targets --all-features -- -D warnings`。
- Mission-Critical 且触碰用户文件、DB、staging、recovery、sync、import、migration、reindex 等边界时，`./dev check task <label>` 会升级到 Core quality gate。
- Stage/foundation closeout、release 或 manifest 显式要求 broad gate 时才使用 `./dev check all`。非 stage task 不应把 `./dev check all` 当默认验收命令。
- 若 Core task 没有 targeted test mapping，应判定为验证映射缺口；不要静默退回 `cargo test --workspace`，除非人工显式设置 emergency fallback。
