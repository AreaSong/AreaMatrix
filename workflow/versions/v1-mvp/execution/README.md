...
python3 workflow/versions/v1-mvp/execution/_shared/prompt_pipeline.py plan --phase phase-0
python3 workflow/versions/v1-mvp/execution/_shared/prompt_pipeline.py plan --all
python3 workflow/versions/v1-mvp/execution/_shared/prompt_pipeline.py next
python3 workflow/versions/v1-mvp/execution/_shared/prompt_pipeline.py render --task 0-1/task-01
python3 workflow/versions/v1-mvp/execution/_shared/prompt_pipeline.py render --task 0-1/task-01 --mode verify
python3 workflow/versions/v1-mvp/execution/_shared/prompt_pipeline.py verify --task 0-1/task-01
python3 workflow/versions/v1-mvp/execution/_shared/prompt_pipeline.py verify --phase phase-0
python3 workflow/versions/v1-mvp/execution/_shared/prompt_pipeline.py export --phase phase-0
python3 workflow/versions/v1-mvp/execution/_shared/prompt_pipeline.py export --all
python3 workflow/versions/v1-mvp/execution/_shared/prompt_pipeline.py mark --task 0-1/task-01 --status completed
...
```text
workflow/versions/v1-mvp/execution/_shared/copy-ready/phase-0..phase-4/*.md
workflow/versions/v1-mvp/execution/_shared/verify-ready/phase-0..phase-4/*.md
```
...
```bash
python3 workflow/versions/v1-mvp/execution/_shared/prompt_pipeline.py mark --task 0-1/task-01 --status in_progress
python3 workflow/versions/v1-mvp/execution/_shared/prompt_pipeline.py mark --task 0-1/task-01 --status completed
```

进度写入本地文件 `workflow/versions/v1-mvp/execution/_shared/progress.json`，默认不提交。当前 v1 状态已完成，`next` / `status` 主要用于审计历史队列。
...
`workflow/` 是大功能 / 版本 / 重构 / 优化的生命周期系统；`workflow/versions/v1-mvp/execution/**` 是已完成的 v1 历史队列，也是未来经显式 promotion 后才会写入的新 live queue；`./task-loop` 只执行已批准 tasks，不做需求决策。

- `workflow/versions/v1-mvp/` 记录已完成的 637-task MVP archive；不要重写现有 `workflow/versions/v1-mvp/execution/**`、`progress.json` 或 task-loop evidence。
...
`v-template` 是模板验收实例，`apply --write` 必须被硬门禁拦住。即使使用 promotion preview `--write`，也只写 promotion review artifact，不修改 `workflow/versions/v1-mvp/execution/**`、不修改 `progress.json`、不启动 `./task-loop`。

这些文件只是 review artifact，不是正式 v1 live queue。它们不会修改 `workflow/versions/v1-mvp/execution/**`、不会更新 `progress.json`，也不会启动或影响 `./task-loop`。后续若要创建真实产品版本，需要单独 `./dev workflow init --version vN`、讨论、计划和验收。
...
通过前仍不得写入 live `workflow/versions/v1-mvp/execution/**`。
...
| Phase 4 | Stage 1 MVP 历史内部 Stage 2/3/4 精细化任务：C2/C3/C4 每个能力拆 5 步，S2/S3/S4 多能力页面拆为 S+C page-feature，并保留 page / stage integration verify |
...
5. Phase 4 按 `4-1`、`4-2`、`4-3` 串行推进：每段先完成 Stage 1 MVP 历史内部 C2/C3/C4 Core 五步任务，再执行 S2/S3/S4 page-feature task 和 page integration verify，最后跑阶段 integration verify。
