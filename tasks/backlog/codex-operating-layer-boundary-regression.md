...
`tasks/prompts/**` live queue，不写 progress、logs、run summaries、runner lock 或
Git checkpoint 状态。
...
| Execution 污染 | PASS | `tasks/backlog/README.md`、`.ai-governance/workflows/prompt-task-runtime.md` | live execution 仍是 `tasks/prompts/**` + `./dev` + `./task-loop`；backlog 不执行 prompt，不启动第二 runner。 |
