...
- `./dev + ./task-loop + workflow/versions/<version>/execution/**` 是唯一 live execution 主线。
...
- 没有修改 `workflow/versions/<version>/execution/**` 任务定义或 manifest。
...
- 若发现 `workflow/versions/<version>/execution/**` 被改动用于本任务，判定不通过。
- 若主线边界仍含糊，判定不通过。
