...
- backlog prompt 不进入 `workflow/versions/<version>/execution/**`，不写 progress。
- `areamatrix-workflow-planning` 是 owner；没有新增重复 `writing-plans` skill。
- 未修改 Vibe-Skills 仓库。
- workflow promotion 前不会写 live queue、checkpoint、run summary、runner lock 或 `workflow/versions/<version>/execution/_shared/progress.json`。
...
```bash
git diff --name-only
rg -n "planning|计划|handoff|copy-ready|verify-ready|source of truth|owner|验证命令|rollback|blocked|workflow/versions/<version>/execution|progress" workflow .codex/references .codex/skills-src tasks/backlog
./dev check skills
...
若 planning 产物可以绕过 `workflow/` gate 直接进入 live `workflow/versions/<version>/execution/**`，判定不通过。
