...
- State 污染检查有证据。
- Skill 污染检查有证据。
- 未修改 `workflow/versions/<version>/execution/**`、progress、runner state 或 checkpoint。

## 必须读取

1. `AGENTS.md`
2. `workflow/versions/v1-mvp/execution/README.md`
...
git diff --stat -- workflow/versions/v1-mvp/execution workflow/versions/v1-mvp/execution/_shared/progress.json .codex/runtime/task-loop/logs workflow/versions/v1-mvp/evidence/task-loop-runs
rg -n "Vibe|Automations|Cloud|Worktrees|hooks|subagent|progress|workflow/versions/<version>/execution|checkpoint|runner|source of truth|源事实" .ai-governance .codex tasks/backlog scripts/dev_tools scripts/task_loop
./dev backlog list
...
若 `workflow/versions/<version>/execution/**` 因本轮工作被修改，判定不通过。
