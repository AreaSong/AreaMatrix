...
10. 当前 637 个任务是 `v1-mvp` live queue；新增 v2 需求先写入 `workflow/versions/v2/changes/*.yaml`，通过 `./dev changes doctor|preview` 检查和预览，不直接改 live queue。
...
`workflow/versions/` 用于记录 prompt workflow 的版本边界：

- `workflow/versions/v1-mvp/` 只是当前 637-task 队列的归档占位，不移动现有 `tasks/prompts/**`。
- `workflow/versions/v2/changes/*.yaml` 用于记录后续新增功能、模块、依赖、精确 docs、docs/API/UDL 同步目标、风险边界和预期 task split。
...
它不会生成 copy-ready / verify-ready 文件，不会修改 manifest，也不会启动或影响 `./task-loop`。
