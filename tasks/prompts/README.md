...
`./dev` 是日常控制台首页：默认彩色，顶部是任务看板，底部是快捷键。`./dev --once` 渲染一次首页后退出，便于截图或脚本检查。颜色可用 `./dev --color never` 或 `NO_COLOR=1` 关闭；完整进程命令用 `./dev processes` 查看。旧版长输出保留在 `./dev status --verbose`。
