...
def print_copy_prompt(task: TaskFile, entry: ManifestEntry) -> None:
    ...
    print("7. 逐个读取当前存在的 `Existing Code`。")
    print("8. 确认改动只会落在 `Expected New Paths`。")
    print("9. 确认不会触碰 `Forbidden Touches`。")
    print("10. 若风险等级为 High 或 Mission-Critical，先给出风险、验证和回滚思路，再继续。")
    print()
    print("## 本次任务标题")
    print()
    print(f"- {task.title}")
...
def print_phase_verify_prompt(phase: str, tasks: dict[str, TaskFile], manifests: dict[str, ManifestEntry]) -> int:
    ...
    print(f"2. 读取依赖关系：`{DEPENDENCY_GRAPH}`")
    print(f"3. 读取 phase manifest：`{MANIFEST_ROOT / (normalized + '.md')}`")
    print("4. 按下方顺序逐个验收 task。")
    print("5. 每个 task 都必须回到 task 文件、manifest 章节、实际文件三者交叉验收。")
    print("6. 任一 task 不通过，则阶段不通过。")
    print()
    print("## 阶段任务清单")
    print()
    for label in labels:
