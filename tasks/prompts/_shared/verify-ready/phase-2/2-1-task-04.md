...
`scripts/check-macos-tests.sh` runs the standard `xcodebuild test` command first.
Only when the failure is explicitly the local `testmanagerd` sandbox restriction
does it reuse the built XCTest bundle with `xcrun xctest`; compile failures,
assertion failures, linker failures, and non-sandbox test failures still fail.
