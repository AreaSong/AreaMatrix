...
6. 读取依赖关系：`/Users/as/Ai-Project/project/AreaMatrix/tasks/prompts/_shared/dependency-graph.md`
7. 读取 phase manifest：`/Users/as/Ai-Project/project/AreaMatrix/tasks/prompts/_shared/manifests/phase-4.md`
8. 在 manifest 中定位章节：`## 4-3/task-160`
9. 逐个读取该章节下的 `Exact Docs`。
10. 逐个读取当前存在的 `Existing Code`。
11. 确认改动只会落在 `Expected New Paths`。
12. 确认不会触碰 `Forbidden Touches`。
13. 若风险等级为 High 或 Mission-Critical，先给出风险、验证和回滚思路；若自动执行器已注入静默授权，则记录后直接继续。
