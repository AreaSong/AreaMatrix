...
def _build_parser() -> argparse.ArgumentParser:
    ...
    codex_os_start_flow = codex_os_sub.add_parser("start-flow", help="Run the 0-100 task start flow")
    add_codex_os_common(codex_os_start_flow)
    codex_os_start_flow.add_argument("--task-id", help="Existing or new task id")
    codex_os_start_flow.add_argument("--title", help="Create a new local registry task when no matching task exists")
    codex_os_start_flow.add_argument("--project-name", default="AreaMatrix", help="Project display name for new tasks")
    codex_os_start_flow.add_argument("--lane", choices=CODEX_OS_LANES, help="Lane for new task or default lookup")
    codex_os_start_flow.add_argument("--owner-thread", help="Owner Codex thread id for new tasks")
    codex_os_start_flow.add_argument("--handoff-file", help="Handoff file path for new tasks")
    codex_os_start_flow.add_argument("--next-action", help="Next action text for new tasks")
    codex_os_start_flow.add_argument("--path", action="append", default=[], help="Changed or expected path; may be repeated")
    codex_os_start_flow.add_argument("--changed", action="store_true", help="Include current git status paths")
    codex_os_start_flow.add_argument("--strict", action="store_true", help="Return non-zero when start preflight fails")
    add_codex_os_task_fields(codex_os_start_flow)
    codex_os_start_flow.add_argument("--json", action="store_true", help="Print JSON instead of Markdown")
    codex_os_start_flow.add_argument("--write", action="store_true", help="Create/start registry task and write start-flow artifacts")
    codex_os_flow = codex_os_sub.add_parser("flow", help="Run the aggregate 0-100 Codex OS task flow")
    add_codex_os_common(codex_os_flow)
    codex_os_flow.add_argument("--task-id", help="Existing or new task id")
    codex_os_flow.add_argument("--title", help="Create a new local registry task when no matching task exists")
    codex_os_flow.add_argument("--project-name", default="AreaMatrix", help="Project display name for new tasks")
    codex_os_flow.add_argument("--lane", choices=CODEX_OS_LANES, help="Lane for new task or default lookup")
    codex_os_flow.add_argument("--owner-thread", help="Owner Codex thread id for new tasks")
    codex_os_flow.add_argument("--handoff-file", help="Handoff file path for new tasks")
    codex_os_flow.add_argument("--next-action", help="Next action text for new tasks")
    codex_os_flow.add_argument("--path", action="append", default=[], help="Changed or expected path; may be repeated")
    codex_os_flow.add_argument("--changed", action="store_true", help="Include current git status paths")
    codex_os_flow.add_argument("--profile", choices=CODEX_OS_VALIDATION_PROFILES, default="standard", help="Validation breadth profile")
    codex_os_flow.add_argument("--execute", action="store_true", help="Run allowlisted validation commands")
    codex_os_flow.add_argument("--timeout", type=int, default=600, help="Seconds per command when --execute is used")
    codex_os_flow.add_argument("--close-when-pass", action="store_true", help="Close the task only when executed validation passes")
    codex_os_flow.add_argument("--ops", action="store_true", help="Append advisory ops-flow output")
    codex_os_flow.add_argument("--strict", action="store_true", help="Return non-zero when the aggregate flow is not ready")
    codex_os_flow.add_argument("--archive-recommendation", choices=CODEX_OS_ARCHIVE_RECOMMENDATIONS, default="keep")
    add_codex_os_task_fields(codex_os_flow)
    codex_os_flow.add_argument("--json", action="store_true", help="Print JSON instead of Markdown")
    codex_os_flow.add_argument("--write", action="store_true", help="Write aggregate flow artifacts and child flow artifacts")
    codex_os_go = codex_os_sub.add_parser("go", help="Low-friction safe wrapper for the aggregate Codex OS flow")
    add_codex_os_common(codex_os_go)
    codex_os_go.add_argument("--task-id", help="Existing or new task id")
    codex_os_go.add_argument("--title", help="Create a new local registry task when no matching task exists")
    codex_os_go.add_argument("--project-name", default="AreaMatrix", help="Project display name for new tasks")
    codex_os_go.add_argument("--lane", choices=CODEX_OS_LANES, help="Lane for new task or default lookup")
    codex_os_go.add_argument("--owner-thread", help="Owner Codex thread id for new tasks")
    codex_os_go.add_argument("--handoff-file", help="Handoff file path for new tasks")
    codex_os_go.add_argument("--next-action", help="Next action text for new tasks")
    codex_os_go.add_argument("--path", action="append", default=[], help="Changed or expected path; may be repeated")
    codex_os_go.add_argument("--changed", action="store_true", help="Include current git status paths")
    codex_os_go.add_argument("--profile", choices=CODEX_OS_VALIDATION_PROFILES, default="auto", help="Validation breadth profile")
    codex_os_go.add_argument("--apply", action="store_true", help="Execute validation and write safe Codex OS runtime artifacts")
    codex_os_go.add_argument("--timeout", type=int, default=600, help="Seconds per command when --apply is used")
    codex_os_go.add_argument("--close-when-pass", action="store_true", help="Close the task only when executed validation passes")
    codex_os_go.add_argument("--ops", action="store_true", help="Append advisory ops-flow output")
    codex_os_go.add_argument("--strict", action="store_true", help="Return non-zero when the aggregate flow is not ready")
    codex_os_go.add_argument("--archive-recommendation", choices=CODEX_OS_ARCHIVE_RECOMMENDATIONS, default="keep")
    add_codex_os_task_fields(codex_os_go)
    codex_os_go.add_argument("--json", action="store_true", help="Print JSON instead of Markdown")
    codex_os_run_validation = codex_os_sub.add_parser("run-validation", help="Recommend or execute selected validation commands")
    add_codex_os_common(codex_os_run_validation)
    codex_os_run_validation.add_argument("--task-id", help="Task id whose validation field should be included")
    codex_os_run_validation.add_argument("--path", action="append", default=[], help="Changed path; may be repeated")
    codex_os_run_validation.add_argument("--changed", action="store_true", help="Include current git status paths")
    codex_os_run_validation.add_argument("--profile", choices=CODEX_OS_VALIDATION_PROFILES, default="standard", help="Validation breadth profile")
    codex_os_run_validation.add_argument("--execute", action="store_true", help="Run allowlisted validation commands")
    codex_os_run_validation.add_argument("--timeout", type=int, default=600, help="Seconds per command when --execute is used")
    codex_os_run_validation.add_argument("--json", action="store_true", help="Print JSON instead of Markdown")
    codex_os_run_validation.add_argument("--write", action="store_true", help="Write validation report and update registry when task-id is set")
    codex_os_repair_plan = codex_os_sub.add_parser("repair-plan", help="Build a read-only repair plan from diagnosis and validation results")
    add_codex_os_common(codex_os_repair_plan)
    codex_os_repair_plan.add_argument("--task-id", help="Task id to diagnose")
    codex_os_repair_plan.add_argument("--path", action="append", default=[], help="Changed path for validation recommendation")
    codex_os_repair_plan.add_argument("--changed", action="store_true", help="Include current git status paths")
    codex_os_repair_plan.add_argument("--validation-report", help="Validation report JSON path; defaults to latest runtime report")
    codex_os_repair_plan.add_argument("--json", action="store_true", help="Print JSON instead of Markdown")
    codex_os_repair_plan.add_argument("--write", action="store_true", help="Write repair-plan.md and repair-plan.json")
    codex_os_close_flow = codex_os_sub.add_parser("close-flow", help="Write evidence/closeout and finish a local Codex OS task")
    add_codex_os_common(codex_os_close_flow)
    codex_os_close_flow.add_argument("--task-id", required=True)
    codex_os_close_flow.add_argument("--status", choices=["Done", "Blocked", "Abandoned"], required=True)
    close_flow_validation = codex_os_close_flow.add_mutually_exclusive_group()
    close_flow_validation.add_argument("--validation", help="Fresh validation command/result summary")
    close_flow_validation.add_argument("--from-latest-validation", action="store_true", help="Use the latest executed PASS report for this task")
    codex_os_close_flow.add_argument("--handoff-file", help="Handoff file path")
    codex_os_close_flow.add_argument("--next-action", help="Next action when blocked or follow-up remains")
    codex_os_close_flow.add_argument("--archive-recommendation", choices=CODEX_OS_ARCHIVE_RECOMMENDATIONS, default="keep")
    add_codex_os_task_fields(codex_os_close_flow)
    codex_os_close_flow.add_argument("--json", action="store_true", help="Print JSON instead of Markdown")
    codex_os_close_flow.add_argument("--write", action="store_true", help="Write evidence/closeout artifacts and update registry")
    codex_os_done = codex_os_sub.add_parser("done", help="Low-friction safe wrapper for close-flow Done")
    add_codex_os_common(codex_os_done)
    codex_os_done.add_argument("--task-id", required=True)
    codex_os_done.add_argument("--preview", action="store_true", help="Preview instead of writing Done closeout")
    codex_os_done.add_argument("--archive-recommendation", choices=CODEX_OS_ARCHIVE_RECOMMENDATIONS, default="keep")
    add_codex_os_task_fields(codex_os_done)
    codex_os_done.add_argument("--json", action="store_true", help="Print JSON instead of Markdown")
    codex_os_ops_flow = codex_os_sub.add_parser("ops-flow", help="Run advisory Codex OS operations review")
    add_codex_os_common(codex_os_ops_flow)
    codex_os_ops_flow.add_argument("--limit", type=int, default=50, help="Rows per advisory group")
    codex_os_ops_flow.add_argument("--strict", action="store_true", help="Return non-zero when operations health fails")
    ops_flow_mode = codex_os_ops_flow.add_mutually_exclusive_group()
    ops_flow_mode.add_argument("--compact", action="store_true", help="Render a compact operations summary")
    ops_flow_mode.add_argument("--action-items", action="store_true", help="Render only advisory action items")
    codex_os_ops_flow.add_argument("--json", action="store_true", help="Print JSON instead of Markdown")
    codex_os_ops_flow.add_argument("--write", action="store_true", help="Write ops-flow.md and ops-flow.json")
    codex_os_todo = codex_os_sub.add_parser("todo", help="Show advisory Codex OS operations action items")
    add_codex_os_common(codex_os_todo)
    codex_os_todo.add_argument("--limit", type=int, default=50, help="Rows per advisory group")
    codex_os_todo.add_argument("--strict", action="store_true", help="Return non-zero when operations health fails")
    codex_os_todo.add_argument("--json", action="store_true", help="Print JSON instead of Markdown")
    codex_os_todo.add_argument("--write", action="store_true", help="Write ops-flow.md and ops-flow.json")
    codex_os_now = codex_os_sub.add_parser("now", help="Show a single-page Codex OS task readiness summary")
    add_codex_os_common(codex_os_now)
    codex_os_now.add_argument("--task-id", help="Task id to inspect; defaults to first non-terminal task")
    codex_os_now.add_argument("--lane", choices=CODEX_OS_LANES, help="Limit default lookup to one lane")
    codex_os_now.add_argument("--json", action="store_true", help="Print JSON instead of Markdown")
    codex_os_now.add_argument("--write", action="store_true", help="Write now.md and now.json")
    codex_os_subagent_plan = codex_os_sub.add_parser("subagent-plan", help="Recommend read-only subagent lanes without spawning agents")
    add_codex_os_common(codex_os_subagent_plan)
    codex_os_subagent_plan.add_argument("--task-id", help="Task id to include")
    codex_os_subagent_plan.add_argument("--lane", choices=CODEX_OS_LANES, help="Lane override when no registered task is used")
    codex_os_subagent_plan.add_argument("--risk-level", choices=CODEX_OS_RISK_LEVELS, help="Risk override for delegation planning")
    codex_os_subagent_plan.add_argument("--path", action="append", default=[], help="Changed or expected path; may be repeated")
    codex_os_subagent_plan.add_argument("--changed", action="store_true", help="Include current git status paths")
    codex_os_subagent_plan.add_argument("--json", action="store_true", help="Print JSON instead of Markdown")
    codex_os_subagent_plan.add_argument("--write", action="store_true", help="Write subagent-plan.json")
