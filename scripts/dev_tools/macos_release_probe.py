"""macOS release app launch probe helpers."""
...
RELEASE_APP_LAUNCH_BLOCKED = 75


def _release_launch_probe_build_base(root: Path, build_base: Sequence[str]) -> list[str]:
...
def run_release_app_launch_probe(
    root: Path,
    ...
    print()
    print("==> xcodebuild build signed Release for release app launch probe")
    release_build_base = _release_launch_probe_build_base(root, build_base)
    ...
    _require_release_probe_inputs(probe_script, app_bundle)
    ...
    if rc != 0:
        return rc
    return _run_release_swift_launch_probe(derived_data_dir, probe_script, app_bundle)


def _require_release_probe_inputs(probe_script: Path, app_bundle: Path) -> None:
    if not probe_script.is_file():
        fail(f"release app launch probe script not found at {probe_script}.")
...
def _run_release_swift_launch_probe(derived_data_dir: Path, probe_script: Path, app_bundle: Path) -> int:
    sdk = subprocess.check_output(["xcrun", "--sdk", "macosx", "--show-sdk-path"], text=True).strip()
    module_cache = derived_data_dir / "SwiftProbeModuleCache.noindex"

    print()
    print(f"==> xcrun swift {probe_script} --app {app_bundle}")
    probe = _run_release_swift_probe_process(sdk, module_cache, probe_script, app_bundle)
    if probe.stdout:
        sys.stdout.write(probe.stdout)
    if probe.returncode == 0:
        return 0
    if _launchservices_probe_blocked(probe.stdout or ""):
        print("release real .app launch probe: BLOCKED by local LaunchServices sandbox.")
        print("release real .app launch probe: retrying signed Release executable directly.")
        return _run_release_direct_launch_probe(derived_data_dir, probe_script, app_bundle)
    return probe.returncode


def _run_release_direct_launch_probe(derived_data_dir: Path, probe_script: Path, app_bundle: Path) -> int:
    sdk = subprocess.check_output(["xcrun", "--sdk", "macosx", "--show-sdk-path"], text=True).strip()
    ...
    probe = _run_release_swift_probe_process(
        sdk,
        module_cache,
        probe_script,
        app_bundle,
    ...
    if _direct_launch_probe_blocked(probe.stdout or ""):
        print("release real .app direct executable probe: BLOCKED by local windowing sandbox.")
        return RELEASE_APP_LAUNCH_BLOCKED
    return probe.returncode


def _run_release_swift_probe_process(
    sdk: str,
    module_cache: Path,
    probe_script: Path,
    app_bundle: Path,
    ...
    return subprocess.run(
        _release_swift_probe_argv(sdk, module_cache, probe_script, app_bundle, extra_args),
        check=False,
        text=True,
...
def _verify_release_app_is_self_contained(app_bundle: Path) -> int:
    ...
    if "libarea_matrix_core.dylib" in (probe.stdout or ""):
        print("release real .app launch probe: FAIL; Release app links core dylib instead of staticlib.")
        return 1
    return 0
